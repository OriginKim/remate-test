...
public class ReceiptParser {
    ...
    public static int extractTotalAmount(String text) {
        ...
        Pattern p1 = Pattern.compile("(?:합계|결제금액|승인금액|총금액|받을금액|영수금액|TOTAL|AMOUNT)[:\\s]*(\\d{3,8})", Pattern.CASE_INSENSITIVE);
    ...
    public static String extractTradeDate(String text) {
        Pattern p = Pattern.compile("(20\\d{2})[\\./\\-\\s](0[1-9]|1[0-2])[\\./\\-\\s](0[1-9]|[12][0-9]|3[01])");
    ...
    public static String extractStoreName(String text) {
        ...
        for (int i = 0; i < Math.min(lines.length, 10); i++) {
            ...
            if (line.matches(".*(고객용|영수증|매출|전표|카드|번호|전화|주소|가맹|신고|포상금|금융|협회|Smartro|결제|승인|사업자).*")) continue;

            // 상호명에 포함된 불필요한 숫자/기호 제거 (예: 23 군자농협 -> 군자농협)
            return line.replaceAll("^[0-9\\s]+", "") // 앞쪽 숫자 제거
                    .replace("상호:", "").replace("상호", "")
                    .trim();
        }
        return "알 수 없는 상호";
    }
